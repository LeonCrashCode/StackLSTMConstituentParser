...
static bool IsActionForbidden_Generative(const string& a, char prev_a, unsigned bsize, unsigned ssize, unsigned nopen_parens, unsigned unary) {
...
//  if (is_nt && nopen_parens > MAX_OPEN_NTS) return true;
  ...
  if (is_nt) {
    if(bsize == 0 && unary >= MAX_UNARY) return true;
...
if(DEBUG) cerr<< "unary: " << unary << "nopen_parens: "<<nopen_parens<<"\n";      
      for (auto a: possible_actions) {
        if (IsActionForbidden_Generative(adict.Convert(a), prev_a, bsize/*terms.size()*/, stack.size(), nopen_parens, unary))
          continue;
...
if(DEBUG){
      ...
      if (sample) {
        ...
        //cerr << endl;
        ++action_count;
	auto adist = as_vector(hg->incremental_forward());
	double best_score = adist[current_valid_actions[0]];
        action = current_valid_actions[0];
...
int main(int argc, char** argv) {
  ...
  parser::TopDownOracleGen test_corpus(&termdict, &adict, &posdict, &ntermdict);
